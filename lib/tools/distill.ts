...
const DISTILL_DESCRIPTION = `Use this tool to distill relevant findings from a selection of raw tool outputs into preserved knowledge, in order to denoise key bits and parts of context.
            ...
            targets: tool.schema
                .array(
                    tool.schema.object({
                        id: tool.schema
                            .string()
                            .describe("Numeric ID (as string) from the <prunable-tools> list"),
                        distillation: tool.schema
                            .string()
                            .describe("Complete technical substitute for that tool output"),
                    }),
                )
                .describe("Array of targets to distill, each with id and distillation text"),
        },
        async execute(args, toolCtx) {
            const targets = args.targets as Array<{ id: string; distillation: string }>
            const ids = targets.map((t) => t.id)
            const distillations = targets.map((t) => t.distillation)
